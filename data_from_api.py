...
for i in range(len(data['data']['profile']['positions'])):
    ...
    arr_ach.append(summary)

# arr_responsibility_achievement = []

# for i in range(len(data['data']['profile']['positions'])):
#     summary = data['data']['profile']['positions'][i]['summary']

    # try:
    #     responsibility = summary.split('Main responsibilities')[1].split('The most relevant results')[0]
    # except:
    #     responsibility = 'NA'

    # try:
    #     achievement = summary.split('Main responsibilities')[1].split('The most relevant results')[1][:-57]
    # except:
    #     achievement = 'NA'
    
    # arr_responsibility_achievement.append([responsibility, achievement])

# position_summary = data['data']['profile']['positions']

# # key_responsibilities, key_achievements = extract_responsibilities_and_achievements(position_summary)
# arr_key_responsibilities = []
# arr_key_ach = []

# for i in position_summary:
#   arr_key_responsibilities.append(i['summary'])
#   arr_key_responsibilities.append(i['achievements'])
